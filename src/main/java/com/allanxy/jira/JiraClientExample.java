...
public class JiraClientExample {
    ...
    String jiraUrl = "http://183.62.253.123:9990";


    String issueUrl = jiraUrl +"/browse/";


    String issuesID = "PRODEPLOY-118";
    ...
    public static void main(String[] args){
        ...
        try {
            new JiraClientExample().doquery1();    //.doing();
    ...
    void doquery1() throws JiraException{
        JiraClientUtil jcu = JiraClientUtil.getInstance(jiraUrl, "cocoweb", "cocoweb,1");
        
        


        /* Retrieve issue TEST-123 from JIRA. We'll get an exception if this fails. */
         issue = jcu.getJiraClient().getIssue(issuesID);
//         List<Issue> listissue= issue.getSubtasks();
//         
//         for(Issue is :listissue){
//             if(is!=null){
//                 JiraClientUtil.showLinks(is);
//             }
//         }
//         
//         JiraClientUtil.showLinks(issue);
        Map<String, String> omap = JiraClientUtil.getIssueAllinks(issue);
    ...
    private void doing() throws JiraException{
        JiraClientUtil jcu = JiraClientUtil.getInstance(jiraUrl, "cocoweb", "cocoweb,1");
        jira = jcu.getJiraClient();

        try {
            /* Retrieve issue TEST-123 from JIRA. We'll get an exception if this fails. */
             issue = jira.getIssue(issuesID);
